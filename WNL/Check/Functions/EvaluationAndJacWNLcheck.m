function [J,Eval] = EvaluationAndJacWNLcheck(PsiE, ThetaE, Ra, Pr,G,n,m,N,positionMatrix)
    len = length(PsiE);
    ...
    K4 = K2.^2;
    Eval1 = NLpsi + (G/2)*(Ra*kx.*ThetaE + K4.*PsiE); % removed sign vector
    Eval2 = NLtheta + (G/2)*(kx.*PsiE/Pr + K2.*ThetaE/Pr);
    Eval = [Eval1; Eval2];
    % jacob
    J = J + (G/2)*[diag(K4) diag(Ra*kx); diag(kx/Pr) diag(K2/Pr)];
end
