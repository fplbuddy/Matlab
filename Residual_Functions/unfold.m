function substruct = unfold(SC,substruct,varargin)
    ...
    for h=1:hmax
        ...
        for i=1:NF
            ...
            if isstruct(SC(h).(F{i}))
                substruct = unfold(SC(h).(F{i}),substruct,Namei,show);
            else
                if iscell(SC(h).(F{i}))
                    siz = size(SC(h).(F{i}));
                    NC = numel(SC(h).(F{i}));
...
elseif iscell(SC)
    ...
    for i=1:numel(SC)
        ...
        substruct = unfold(SC{i},substruct,Namei,show);
    end
else
    disp(Name)
    if show
