...
clear h
figure()
h(1) = loglog(Pr_list,data.fenk,'.r','DisplayName','$B$','MarkerSize',MS), hold on
h(2) = loglog(Pr_list,nu_list.*data.denk,'dr','DisplayName','$\nu S$','MarkerSize',MS*0.3,'MarkerFaceColor','r')
h(3) = loglog(Pr_list,hnu*data.henk,'sr','DisplayName','$\mu L$','MarkerSize',MS*0.3,'MarkerFaceColor','r')
xlabel('$Pr$')
...
saveas(gcf,[figpath 'KineticEnergyBudgetconstnu_' tit], 'epsc')
...
%% add to fig
figure()
h(1) = loglog(Pr_list,data.fenk,'.b','DisplayName','$B$','MarkerSize',MS), hold on
h(2) =loglog(Pr_list,nu_list.*data.denk,'db','DisplayName','$\nu S$','MarkerSize',MS*0.3,'MarkerFaceColor','b')
h(3) =loglog(Pr_list,hnu*data.henk,'sb','DisplayName','$\mu L$','MarkerSize',MS*0.3,'MarkerFaceColor','b')
xlabel('$Pr$')

% fit the power law
[alpha, A, xFitted, yFitted, ~] = FitsPowerLaw(Pr_list,nu_list.*data.denk);
plot(xFitted(30:80),yFitted(30:80)*5e-1,'--k');
ylim([1e-4 1])
text(1,1e-3,['$Pr^{' num2str(alpha,'%.2f') '}$'])
...
saveas(gcf,[figpath 'KineticEnergyBudgetconstkappa_' tit], 'epsc')
