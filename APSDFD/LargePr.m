...
for i=1:length(phase_list)
    ...
    %subplot(1,2,1)
    set(subplot(1,2,1), 'Position', [0.08, 0.15, 0.4, 0.8])
    ...
    % make text
    text(1.2e5,2.47e7,'$E_{PD}$', 'FontSize', LabelFS, 'Color', 'green' )
    text(1.4e5,2e7,'$E_{SB}$', 'FontSize', LabelFS, 'Color', 'r' )
    xq = 6e5;
    yq = 2.7e7;
    
    arrow([3e5 1.7e7],[1.05e5 1.57e7],5, 'Color',myBlue)
    arrow([7e5 1.8e7],[7.5e5 2.53e7],5, 'Color',myBlue)
    ...
    xticks([])
    yticks([])
    set(subplot(1,2,2), 'Position', [0.57, 0.15, 0.4, 0.8])
    % run ModePlot_v3 first
    loglog(Pr_list_energy, energy_list, '.', 'Color','r','MarkerSize',MarkerS+10), hold on
    
    yticks([1e-6 1e-4 1e-2 1])
    xticks([10 1e2 1e3 1e4 1e5 1e6])
    xticklabels(["$10^1$" "$10^2$"  "$10^3$" "$10^4$" "$10^5$" "$10^6$"])
    ylim([2e-7 1])
    xlim([10 max(Pr_list_energy)])
    plot([7e5 7e5], [2e-7 1], '--','Color', 'r', 'LineWidth',2)
    ax = gca;
    ax.XAxis.FontSize = numFS;
    ax.YAxis.FontSize = numFS;
    xlabel('$Pr$','FontSize', LabelFS)
    ...
    set(gca,'XColor',[1 1 1])
    set(gca,'YColor',[1 1 1])
    text(1.4e3,6e-3,'$E_{SB}$', 'FontSize', LabelFS, 'Color', 'r' )
    %
