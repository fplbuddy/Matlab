...
%%
Pr_list = [1e4 6e4 1e5 1.1e5 1.3e5 1.6e5 2e5 3e5 1e6];
...
for i=1:length(Pr_list)
    ...
    for j=1:length(RaS_list)
        RaS = RaS_list(j);
        try
        sigmaodd = Data.AR_2.OneOne400.(PrS).(RaS).sigmaodd; % will cause error for some
...
subplot(2,1,2)
(Pr_list, ZeroOne_list, 'r-o')
ax = gca;
ax.XAxis.FontSize = numFS;
ax.YAxis.FontSize = numFS;
xlabel('$Pr$','FontSize', LabelFS)
text(2e4, 1e-10, '$\sum_{k_y} | \widehat \psi_{0,k_y}|$', 'FontSize',LabelFS, 'color', 'red')

text(1.1e4, 1e39, '(a)', 'FontSize',numFS)
text(1.1e4, 1e3, '(b)', 'FontSize',numFS)
