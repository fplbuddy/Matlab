...
Pr_list = [Pr_list 1e4 6e4 7.2e5 1e6]; 
Ra_list = [Ra_list 4.24e6 1.25e7 2.56e7 2.6e7]; % The one for Pr = 1e6 is not strickt right... will remove if needed
% have gotten rid of Pr = 2e4,3e4,3e5 above, will add in manually..
...
% VERY CHEEKY AND ADD FOR 2e4, 3e4, 3e5!!
Pr_list = [Pr_list 2e4 3e4 3e5];
energy_list = [energy_list' 1e-4 6e-5 2e-6];
...
xticks([10 1e2 1e3 1e4 1e5 7e5])
xticklabels(["$10^1$" "$10^2$"  "$10^3$" "$10^4$" "$10^5$" "$q_4$"])
ylim([2e-7 1])
plot([7e5 7e5], [2e-7 1], 'r--')
...
ylabel('$R_{\textrm{zonal}}$','FontSize', LabelFS)
% make text
Pr_list_energy = Pr_list;
