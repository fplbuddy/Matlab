...
for i=1:length(Pr_list)
    ...
    m = max([m max(Energy)]);
    mm = min([mm min(Energy)]);
end
xlim([0 tmin])
yticks([1e-20 1e-18 1e-16 1e-14 1e-12 1e-10 1e-8 1e-6 1e-4 1e-2 1 1e2])
ylim([max([m/10^10 mm]) m])
ax = gca;
...
title(['$ Ra = ' RatoRaT(Ra) '$, $\Gamma = ' num2str(G) '$'],'FontSize',labelFS)
lgnd = legend('Location', 'Bestoutside', 'FontSize', numFS); title(lgnd,'$Pr$', 'FontSize', numFS)
saveas(gcf,[figpath 'OddKineticE_' convertStringsToChars(RaS) '_G_' num2str(G) '.eps'], 'epsc')
...
title(['$ Ra = ' RatoRaT(Ra) '$, $\Gamma = ' num2str(G) '$'],'FontSize',labelFS)
lgnd = legend('Location', 'Bestoutside', 'FontSize', numFS); title(lgnd,'$Pr$', 'FontSize', numFS)
saveas(gcf,[figpath 'EvenPotentialE_' convertStringsToChars(RaS) '_G_' num2str(G) '.eps'], 'epsc')
...
title(['$ Ra = ' RatoRaT(Ra) '$, $\Gamma = ' num2str(G) '$'],'FontSize',labelFS)
lgnd = legend('Location', 'Bestoutside', 'FontSize', numFS); title(lgnd,'$Pr$', 'FontSize', numFS)
saveas(gcf,[figpath 'OddPotentialE_' convertStringsToChars(RaS) '_G_' num2str(G) '.eps'], 'epsc')
