...
%% sqAx
figure('Renderer', 'painters', 'Position', [5 5 540 200])
semilogy(Pr_list2,sqAx,'x')
hold on
sqAxfit = feval(fit(reshape(Pr_list2,length(Pr_list2),1),reshape(sqAx,length(sqAx),1),'pchipinterp'),PrRange);
plot(PrRange, sqAxfit)
ylabel('$H^{*}(Pr)$', 'FontSize',14)
xlabel('$Pr$', 'FontSize',14)
hold off
%% sqas
figure('Renderer', 'painters', 'Position', [5 5 540 200])
plot(Pr_list2,sqax,'x')
hold on
sqaxfit = feval(fit(reshape(Pr_list2,length(Pr_list2),1),reshape(sqax,length(sqax),1),'pchipinterp'),PrRange);
plot(PrRange, sqaxfit)
ylabel('$h^{*}(Pr)$', 'FontSize',14)
