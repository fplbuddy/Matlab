...
RaFit = logspace(floor(log10(min(Ra))), ceil(log10(max(Ra))),100); % Need to fit the lower bound to when we have sheaing
FittedData = A*RaFit.^alpha;
plot(RaFit, FittedData, 'black--');
legend([NS s], 'Non-Shearing', 'Shearing')
legend('Location', 'NorthEast')
gtext(['$1-\frac{E_x}{E} \propto Ra^{'  num2str(alpha,2) '}$'],'FontSize',13,'color', 'black')
hold off
%% Plot Ex/E
figure('Renderer', 'painters', 'Position', [5 5 540 200])
