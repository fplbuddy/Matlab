...
PrS = PrtoPrS(Pr);
RaS = RatoRaS(Ra);
oddeigf = Data.(AR).(type).(PrS).(RaS).oddeigf;
Psif = oddeigf(1:length(oddeigf)/2);
Thetaf = oddeigf(length(oddeigf)/2+1:end);
...
title({'Most unstable odd eigenfunction with', '$Pr = 12$, $Ra = 3 \times 10^5$','$E(k)$'}, 'FontSize', 17)
%
figure('Renderer', 'painters', 'Position', [5 5 400 300])
pcolor(Penergy');
colormap('jet')
...
xlabel('$k_x$', 'FontSize', 16)
ylabel('$k_y$', 'FontSize', 16)
title({'Most unstable odd eigenfunction with', '$Pr = 12$, $Ra = 3 \times 10^5$','$E^{\theta}(k)$'}, 'FontSize', 17)
%%
function two = two(n)
    if n == 0
        two = 1;
