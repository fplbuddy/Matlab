...
%plotevenfunction(PsiE, ThetaE, 32, 2/3)
...
xr = floor(128/3 + 1) + 1;
yr = floor(2*128/3 + 1);
path = '/Volumes/Samsung_T5/Eigs_Prob_ICs/AR_0_67/128x128/Pr_100/Ra_3e4/Spectra/';
...
for j=1:length(Pr_list)
    Pr = Pr_list(j);
    RaList = [1.3e5 1.4e5]; %[4e4 5e4 5.5e4 6e4 7e4];
    Ra = RaList(1);
    RaS = RatoRaS(Ra);
...
Cont = 1;
while dxnew > thres
    if dxnew > dxold % increse prudance if we are going the wrong way;
        thres = thres*4;
        if thres > dxmin
            Cont = 0;
        end
