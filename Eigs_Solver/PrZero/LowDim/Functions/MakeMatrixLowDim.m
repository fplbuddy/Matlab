function M = MakeMatrixLowDim(modPsiE, modThetaE, Ra, Pr)
%     n = [0 -1 1 -1 1]; 
%     m = [1 2 2 2 2];
%     kx = n*2*pi/G;
%     ky = m*pi;
...
%     M(4:5,1:3) = M(4:5,1:3) + psi2;
    M(1,1) = -pi^2*Pr;
    M(2,2) = -5*pi^2*Pr; M(3,3) = -5*pi^2*Pr; M(4,4) = -5*pi^2; M(5,5) = -5*pi^2;
    M(1,2) = (3/2)*pi^2*modPsiE; M(1,3) = (3/2)*pi^2*modPsiE;
    M(2,1) = (1/10)*pi^2*modPsiE; M(3,1) = (1/10)*pi^2*modPsiE;
    M(4,1) = -1i*(1/2)*pi^2*modThetaE; M(5,1) = 1i*(1/2)*pi^2*modThetaE;
    M(4,2) = -1i*pi; M(5,3) = 1i*pi;
    M(2,4) = 1i*(1/(5*pi))*Ra*Pr; M(3,5) = -1i*(1/(5*pi))*Ra*Pr;
end
