function txtfileeigssmallerzero(PrZeroData,RaA, Pr,type,G)
    % Getting data
    ARS = GtoGS(G);
    RaC = RaCfunc(G);
    Ra = RaC + RaA;
    ...
    Eigv =  PrZeroData.(ARS).(type).(PrS).(RaAS).Eigv;
    Eigvconj =  PrZeroData.(ARS).(type).(PrS).(RaAS).Eigvconj;
    ...
    % making dir
    folder = ['/Volumes/Samsung_T5/EigenVec/' convertStringsToChars(ARS) '/' typeF 'x' typeF '/' convertStringsToChars(PrS) '/' convertStringsToChars(RaAS)];
    if ~exist(folder, 'dir')
       mkdir(folder)
    end
    % storing values
