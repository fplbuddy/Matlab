function res = OWWee_nxny(ninst,minst, n,m,Nx,Ny)
% nmax = max(n);
% mmax = max(m);
...
for i=1:length(n)
    ...
    options(7,1) = ninst + n1; options(7,2) = -minst + m1; % Do we need the these and below? just minus of the above, so we can just take abs?
    ...
    for j=1:9
        ...
        %if checkifaddeenew(n2, m2, nmax,mmax)%checkifadd(n1, m1, n2, m2, nmax,mmax)
        if checkifaddeenew2_nxny(n2, m2, Nx,Ny)
            loc = loc + 1;
