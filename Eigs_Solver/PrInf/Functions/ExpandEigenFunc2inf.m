function EigenFunc = ExpandEigenFunc2inf(EigenFuncOld,Nold, Nnew)
...
% or odd
    ...
    EigenFunc = zeros(length(nnew),1);
    for i=1:length(nold)
        noldinst = nold(i);
        moldinst = mold(i);    
        ThetaAdd = EigenFuncOld(i); % Getting thing we want to add
        for j=1:length(nnew)
            nnewinst = nnew(j);
            mnewinst = mnew(j);
