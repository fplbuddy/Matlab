...
%% plot the steps
path = '/Volumes/Samsung_T5/Coherance_NoChangeToZonal/';
figure()
step_list = [2];
for i=1:length(step_list)
    rstep = step_list(i)
    rstepS = normaltoS(rstep,'rstep',1);
    fpath = [path IC '/' res '/' PrS '/' RaS '/' GyS '/' dtS '/' rstepS];
    kenergy2 = importdata([fpath '/Checks/kenergy2.txt']);
...
lgnd = legend();
title(lgnd,'$N$')
saveas(gcf,[figpath 'RanFieldTSCompOld'], 'epsc')